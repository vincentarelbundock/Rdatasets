...
Quake
   ...
   ``3`` < ``8`` < ``23`` < ``22`` < ``6`` < ``13`` < ``7`` < ``21`` <
   ``18`` < ``15`` < ``4`` < ``12`` < ``19`` < ``5`` < ``9`` < ``1`` <
   ``2`` < ``17`` < ``11`` indicating the earthquake on which the
...
soil
   a factor with levels ``0`` and ``1`` giving the soil condition at the
   measuring station, either soil or rock.
...
originally given in Joyner and Boore (1981); are mentioned in Brillinger
(1987); and are analyzed in Davidian and Giltinan (1995).

Source
~~~~~~

Pinheiro, J. C. and Bates, D. M. (2000), *Mixed-Effects Models in S and
S-PLUS*, Springer, New York. (Appendix A.8)
