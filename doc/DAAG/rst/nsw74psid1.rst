...
subset of the ``nswdemo`` dataset for which 1974 earnings are available.
Data are for the male experimental control and treatment groups, in an
...
Likewise, ``nsw74psid1`` (2675 rows) is the subset of the ``nswpsid1``
data, and ``nsw74psid3`` (313 rows) is the subset of the ``nswpsid3``
data, for which 1974 income is available. NB, also, the ``nsw74psidA``
data set.
...
The ``nsw74psidA`` data set (252 rows) was obtained from ``nsw74psid1``
using:
