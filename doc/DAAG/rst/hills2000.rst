...
The record times in 1984 (``hills``) for 35 Scottish hill races, or in
2000 (``hills2000``) for 56 hill races. The ``hills2000`` dataset is the
subset of ``races2000`` for which ``type`` is ``hill``.
...
311-318. [ "corrects" the time for Knock Hill, in the ``hills`` dataset,
from 78.65 to 18.65. It is unclear if this based on the original
records.]
