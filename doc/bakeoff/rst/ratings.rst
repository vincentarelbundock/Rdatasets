...
This dataset has one row per episode and includes ratings plus original
airdates in the UK (by series) and US (by season). This dataset is made
by joining the ``ratings_raw`` and ``seasons_raw`` datasets by both
``series`` and ``episode``.
