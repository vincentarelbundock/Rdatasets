...
``officialTitle``
   a factor with levels ``BISHOP`` ``CATHOLIC PRIEST`` ``CHIEF CLERK``
   ``CIRCUIT JUDGE`` ``ELDER`` ``MARRIAGE OFFICIAL`` ``MINISTER``
   ``PASTOR`` ``REVEREND``
...
``prevcount``
   the number of previous marriages of the person, as listed on the
   application
...
``college``
   the number of years College education, as listed on the application.
   Where no number was listed, this field was left blank, unless less
   than 12 years High School was reported, in which case it was entered
   as 0.
...
``sign``
   the astrological sign, with levels ``Aquarius`` ``Aries`` ``Cancer``
