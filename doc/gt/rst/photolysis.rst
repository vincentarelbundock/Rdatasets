...
rates. The data provided here in ``photolysis`` allows for computations
of photolysis rates (*J*, having units of ``s^-1``) as a function of the
...
l, m, n
   The parameter values given in the ``l``, ``m``, and ``n`` columns can
   be used to calculate the photolysis rate (*J*) as a function of the
...
wavelength_nm, sigma_298_cm2
   ...
   values in ``wavelength_nm`` provide the wavelength of light in
   nanometer units; the ``sigma_298_cm2`` values are paired with the
   ``wavelength_nm`` values and they are in units of
   ``⁠cm^2 molecule^-1⁠``.
...
``peeps``, ``pizzaplace``, ``reactions``, ``rx_addv``, ``rx_adsl``,
``sp500``, ``sza``, ``towny``
