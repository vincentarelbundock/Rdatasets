...
The ``peeps`` dataset contains records for one hundred people residing
in ten different countries. Each person in the table has address
...
state_prov
   The state or province associated with the ``city`` and ``address``.
   This is ``NA`` for individuals residing in countries where
   subdivision data is not needed for generating a valid mailing
   address.
...
phone_number, country_code
   The individual's phone number and the country code associated with
   the phone number.
