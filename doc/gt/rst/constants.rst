...
uncert
   The uncertainty associated with the value. If ``NA`` then the value
   is seen as an 'exact' value (e.g., an electron volt has the exact
   value of 1.602 176 634 e-19 J).
...
Other datasets: ``countrypops``, ``exibble``, ``films``, ``gibraltar``,
