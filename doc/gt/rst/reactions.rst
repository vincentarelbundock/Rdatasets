...
different temperatures (the temperature dependence parameters: ``A``,
``B``, and ``n``). Uncertainty values/factors and temperature limits are
also provided here where information is available.
...
OH_a, OH_b, OH_n
   ...
   molecules^-1 s^-1, ``B`` is in units of K, and ``n`` is
   dimensionless. Any ``NA`` values indicate that data is not available.
...
``peeps``, ``photolysis``, ``pizzaplace``, ``rx_addv``, ``rx_adsl``,
``sp500``, ``sza``, ``towny``
