...
which ``mc()`` had failed to converge. A numeric vector of 50 values, 30
of which are very close to zero, specifically, their absolute values are
less than ``1.5e-15``.
...
a good robust method will treat the 60% “almost zero” values as “good”
data and all other as outliers.
...
newer versions) ``eps1`` was increased, e.g., only by a factor of 10, to
``eps1 = 1e-13``.
