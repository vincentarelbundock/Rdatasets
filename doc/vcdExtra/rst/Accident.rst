...
as ordered, with levels ``Pedestrian`` < ``Bicycle`` < ``Motorcycle`` <
``4-Wheeled`` as Bertin does. This affects the parameterization in
models, so we don't do this directly in the data frame.
