...
(``cigarette``), M (``marijuana``) to refer to the table variables, and
this usage is followed in the examples below.

Background variables include ``sex`` and ``race`` of the respondent
(GR), typically treated as explanatory, so that any model for the full
table should include the term ``sex:race``. Models for the reduced
table, collapsed over ``sex`` and ``race`` are not entirely
