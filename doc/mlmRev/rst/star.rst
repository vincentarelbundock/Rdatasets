...
which was a large-scale, four-year study of the effect of reduced class
size.
...
``cltype``
   class type - a factor with levels ``small``, ``reg`` and ``reg+A``.
   The last level indicates a regular class size with a teachers aide.
...
``clad``
   career ladder position of the teacher - a factor with levels ``NOT``
   ``APPR`` ``PROB`` ``PEND`` ``1`` ``2`` ``3``
...
``schtype``
   school type - a factor with levels ``inner``, ``suburb``, ``rural``
   and ``urban``
...
``yrs``
   number of years of schooling for the student - a numeric version of
   the grade ``gr`` with Kindergarten represented as 0. This variable
   was generated from ``gr`` and does not allow for a student being
   retained.
...
Word, E., Johnston, J., Bain, H.P., et al. (1994). *The State of
Tennessee's Student/Teacher Achievement Ratio (STAR) Project: Technical
Report 1985–1990.* Nashville: Tennessee State Department of Education.
